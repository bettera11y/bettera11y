...
describe("deterministic incremental output", () => {
    it("produces stable diagnostic fingerprints across repeated runs", async () => {
        ...
        const first = await auditIncremental({ changes: [input] }, { rules: strictPreset });
        const second = await auditIncremental({ changes: [input] }, { rules: strictPreset });

        expect(first[0]?.diagnostics.map((d) => d.id)).toEqual(second[0]?.diagnostics.map((d) => d.id));
    });
});
